...
import './App.css'
import { initPolkadotClient, getClient, getWestendApi } from './lib/papi/client'
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      sub = client.finalizedBlock$.subscribe((b: { number: number; hash: string }) => setFinalized({ number: b.number, hash: b.hash }))
      setIsReady(true)
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const tx = api.tx.Balances.transfer_allow_death({ dest: MultiAddress.Id(dest), value })
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ marginTop: 24, display: 'grid', gap: 8, maxWidth: 520 }}>
        ...
        />
        {/* Fee preview removed */}
        <button disabled={!selected || !recipient || !amountWnd} onClick={handleTransfer}>
          Send
        </button>
        {txState && <div>Tx: {txState}</div>}
        {txHash && (
          <a href={`https://westend.subscan.io/extrinsic/${txHash}`} target="_blank" rel="noreferrer">
            View on Subscan
          </a>
        )}
      </div>
